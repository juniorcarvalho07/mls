...
const int n = 128 /*grid resolution (cells)*/, window_size = 800;
const real dt = 0.01 / n , frame_dt = 1e-2_f, dx = 1.0_f / n,
           inv_dx = 1.0_f / dx;
auto particle_mass = 1.0_f, vol = 1.75_f;
...
// environment
  ...
  void init(float jitter = 0.0, float spacing = 0.02) {
   ...
   // vector<Vec> pos = sample_hex(spacing, jitter, Vec(0.55,0.8), Vec(0.95, 0.9));
    ...
    vector<Vec> pos1 = sample_hex(spacing, jitter, Vec(0.25,0.3), Vec(0.75, 0.9));


    for (int i = 0; i < pos1.size(); ++i) {
      particles.push_back(Particle(2,pos1[i]));
    }
  ...
  void step() {
        ...
        std::memset(grid, 0, sizeof(grid));  // Reset grid
          for (auto &p : particles) {          // P2G
            ...
            Mat cauchy;
            if (p.type == 2) {
              cauchy = Mat(0.01_f * E * (pow<1>(p.Jp) - 1));
            } else {
    ...
    for (int i = 0;; i++) {               //              Main Loop
      ...
      if (i % int(frame_dt / dt) == 0) {  //        Visualize frame
        ...
        for (auto p : mpm.particles)
          canvas.circle(p.x).radius(2).color(0xF2B134);  // Particles
