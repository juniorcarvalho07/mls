...
public:
    ...
    template<
            class U,
            class UnqualifiedType = std::remove_cv<T>>
...
//
    ...
    void neighbors(const T& x, std::vector<NeighborData> &nbs,float h2)
    {
      nbs.clear();
      idx_t idx = hash_func(x);
	...
	//this can be improved.  if we already have
        ...
        nn.it = m_bins[idx_cur].end();
        

        for(auto it = m_bins[idx_cur].begin(); it != m_bins[idx_cur].end(); it++)
        {
            const T& t = *it;
            ...
            float cur_dist = dist2(
                t.second,x.second);
                if(cur_dist < h2){
		  dir = (t.second - x.second);
                    NeighborData data;
		    data.idx = t.first;
		    data.d_squared = cur_dist;
                    data.d = sqrt(cur_dist);
		    if (data.d != 0) 
    ...
    {
        idx_t ret;
        my_vect3_t  r = my_vect3_t( t.second);
