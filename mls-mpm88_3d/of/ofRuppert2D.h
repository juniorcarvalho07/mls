...
public:
	...
	*/
	...
	void setStepok(bool b){stepok=b;}
        ...
        *  Usa o bordo de uma triangulação de Delaunay para
        *  determinar o fecho convexo de um conjunto de pontos
        */
        void findConvexHull(ofList<sVertex*> *lv,ofList<int> *chIds);
...
//	void removeExtaVertices(int nv);
	...
	*/
	...
	//space fp(space x,space y){return 5.0*((x-this->cx)*(x-this->cx)+(y-this->cy)*(y-this->cy));}
        ...
        space fp(space x,space y){return 0.0;}
...
template <class _Traits> void ofRuppert2D<_Traits>::findConvexHull(ofList<sVertex*> *lv,ofList<int> *chIds)
{
    //insertBoundBox(lv);
    if(delaunay)
            delete delaunay;
    ...
    {
            for(int i=0;i<3;i++)
                if(itc->getMateId(i)==-1)
                {
                    v1=itc->getVertexId((i+1)%3);
                    v2=itc->getVertexId((i+2)%3);
                    if((v1>0)&&(chIds->inList(v1)==false))
                        chIds->insert(v1);
                    if((v2>0)&&(chIds->inList(v2)==false))
                        chIds->insert(v2);
                }
    }
