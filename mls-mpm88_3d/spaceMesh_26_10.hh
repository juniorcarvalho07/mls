  ...
  void setupDephMap( SPH &sph, bool edgeok=false)
  {
   ...
   // glutPostRedisplay();
    ...
    float maxdimx,maxdimy;
     ...
     ApplyDepthFilter(vSSDMap, Nx+1, Ny+1, 5);

     m_iNumNodeVrts = CalNodeVertex(Nx, Ny, dx, dy,m_vSSMGrid, vSSDMap,Box);

     int edge_num = DetectSilhouetteEdgeVertex(Nx, Ny, dx, dy, vSSDMap, m_vSSPrts, Box);
     ...
     ApplySilhoutteSmoothing(m_vSSVertex, polys, 2);
...
*/
  ...
  int CalNodeVertex(int nx, int ny, double dw, double dh, std::vector<rxSSGrid> *grid, const vector<float> &dgrid,BboxScreenSpace Box)
  {
    int Lnx,Lny,Fnx,Fny;
    Fnx=Box.minx/screenSpace;
    Fny=Box.miny/screenSpace;
    Lnx = (Box.maxx-Box.minx)/screenSpace;
    Lny = (Box.maxy-Box.miny)/screenSpace;
   if(grid->size() != Lnx*Lny){
                    grid->clear();
                    grid->resize(Lnx*Lny);
            }
          ...
          for(int j = 0; j <= Lny; ++j){
                  for(int i = 0; i <= Lnx; ++i){
                          double d = dgrid[Fnx+i+(Fny+j)*(nx+1)];
                          if(d < INF){
                                  ...
                                  if(i != 0){
                                          if(j != 0){
                                                  m_vSSMGrid->at((i-1)+(j-1)*Lnx).node_vrts[2] = vidx;
                                                  m_vSSMGrid->at((i-1)+(j-1)*Lnx).num_nv++;
                                          }
                                          if(j != Lny){
                                                  m_vSSMGrid->at((i-1)+(j)*Lnx).node_vrts[1] = vidx;
                                                  m_vSSMGrid->at((i-1)+(j)*Lnx).num_nv++;
                                          }
                                  }
                                  if(i != Lnx){
                                          if(j != 0){
                                                  m_vSSMGrid->at((i)+(j-1)*Lnx).node_vrts[3] = vidx;
                                                  m_vSSMGrid->at((i)+(j-1)*Lnx).num_nv++;
                                          }
                                          if(j != Lny){
                                                  m_vSSMGrid->at((i)+(j)*Lnx).node_vrts[0] = vidx;
                                                  m_vSSMGrid->at((i)+(j)*Lnx).num_nv++;
                                          }
                                  }
                          }
                          ...
                          if(i != 0){
                                  if(j != 0){
                                          m_vSSMGrid->at((i-1)+(j-1)*Lnx).node_depth[2] = d;
                                  }
                                  if(j != Lny){
                                          m_vSSMGrid->at((i-1)+(j)*Lnx).node_depth[1] = d;
                                  }
                          }
                          if(i != Lnx){
                                  if(j != 0){
                                          m_vSSMGrid->at((i)+(j-1)*Lnx).node_depth[3] = d;
                                  }
                                  if(j != Lny){
                                          m_vSSMGrid->at((i)+(j)*Lnx).node_depth[0] = d;
                                  }
                          }
  ...
  int DetectSilhouetteEdgeVertex(int nx, int ny, double dw, double dh, const vector<float> &dgrid,
                                                                                   const vector<rxSSParticle> &ssprts, BboxScreenSpace Box)
  {
    int Lnx,Lny,Fnx,Fny;
    Fnx=Box.minx/screenSpace;
    Fny=Box.miny/screenSpace;
    ...
    Lny = (Box.maxy-Box.miny)/screenSpace;
          ...
          m_vSSEdge->resize((Lnx)*(Lny+1)+(Lnx+1)*(Lny));
          int yoffset = (Lnx)*(Lny+1);


          for(int j = 0; j <= Lny; ++j){
                  for(int i = 0; i < Lnx; ++i){
                          int i00 = i+Fnx+(j+Fny)*(nx+1);
                          int i01 = (Fnx+i+1)+(j+Fny)*(nx+1);

                          rxSSEdge *e = &m_vSSEdge->at(i+j*(Lnx));

                          e->x0 = Vector(Box.minx+dw*i, Box.miny+dh*j, dgrid[i00]);//e->d0 = dgrid[i00];
                          e->x1 = Vector(Box.minx+dw*(i+1), Box.miny+dh*j, dgrid[i01]);//e->d1 = dgrid[i01];
          ...
          for(int j = 0; j < Lny; ++j){
                  for(int i = 0; i <= Lnx; ++i){
                          int i00 = Fnx+i+(Fny+j)*(nx+1);
                          int i10 = Fnx+i+(Fny+j+1)*(nx+1);

                          rxSSEdge *e = &m_vSSEdge->at(yoffset+i+j*(Lnx+1));

                          e->x0 = Vector(Box.minx+dw*i, Box.miny+dh*j, dgrid[i00]);//e->d0 = dgrid[i00];
                          e->x1 = Vector(Box.minx+dw*i, Box.miny+dh*(j+1), dgrid[i10]);//e->d1 = dgrid[i01];
          ...
          for(int k = 0; k < pnum; ++k){
                  ...
                  if(xp.x < Box.minx || xp.x >= Box.maxx || xp.y < Box.miny || xp.y >= Box.maxy){
                          continue;
                  }
                  ...
                  for(int j = minp[1]; j <= maxp[1]; ++j){
                          for(int i = minp[0]; i <= maxp[0]; ++i){
                                  rxSSEdge &e = m_vSSEdge->at(i-Fnx+(j-Fny)*(Lnx));
                  ...
                  for(int i = minp[0]; i <= maxp[0]; ++i){
                          for(int j = minp[1]; j <= maxp[1]; ++j){
                              int eidx = yoffset+i-Fnx+(j-Fny)*(Lnx+1);
                                  rxSSEdge &e = m_vSSEdge->at(eidx);
                                  if(!e.silhouette) continue;
                                  if(e.depth < xp.z) continue;
    ...
    Lny = (Box.maxy-Box.miny)/screenSpace;
          ...
          for(int i = 0; i <  edges->size(); ++i){
                  ...
                  if(e.x0.z < RX_FEQ_INFM && e.x1.z < RX_FEQ_INFM){
                          ...
                          if(e.x0.z > e.x1.z){
                                  back_node = Fnx+ei0+(ej0+Fny)*(nx+1);
                                  if(i < yoffset){
                                          nn_back_node = (Fnx+ei0 == 0 ? Fnx+ei0 : Fnx+ei0-1)+(ej0+Fny)*(Lnx+1);
                                          l = dw;
                                  }
                                  else{
                                          nn_back_node = Fnx+ei0+(Fny+ej0 == 0 ? Fny+ej0 : Fny+ej0-1)*(Lnx+1);
                                          l = dh;
                                  }
                          }
                          else{
                                  back_node = Fnx+ei1+(Fny+ej1)*(nx+1);
                                  if(i < yoffset){
                                          nn_back_node = (Fnx+ei1 == nx ? Fnx+ei1 : Fnx+ei1+1)+(ej1+Fny)*(Lnx+1);
                                          l = dw;
                                  }
                                  else{
                                          nn_back_node = Fnx+ei1+(Fny+ej1 == ny ? Fny+ej1 : Fny+ej1+1)*(Lnx+1);
                                          l = dh;
                                  }
                          }
                  ...
                  if(e.xy == 0){
                          if(ej0 != 0){
                                  m_vSSMGrid->at(ei0+(ej0-1)*Lnx).edge_vrts[2] = vidx;
                                  m_vSSMGrid->at(ei0+(ej0-1)*Lnx).num_ev++;
                                  if(bidx != -1){
                                          m_vSSMGrid->at(ei0+(ej0-1)*Lnx).back_vrts[2] = bidx;
                                          m_vSSMGrid->at(ei0+(ej0-1)*Lnx).num_bv++;
                                  }
                          }
                          if(ej0 != ny){
                                  m_vSSMGrid->at(ei0+ej0*Lnx).edge_vrts[0] = vidx;
                                  m_vSSMGrid->at(ei0+ej0*Lnx).num_ev++;
                                  if(bidx != -1){
                                          m_vSSMGrid->at(ei0+ej0*Lnx).back_vrts[0] = bidx;
                                          m_vSSMGrid->at(ei0+ej0*Lnx).num_bv++;
                                  }
                          }
                  }
                  else{
                          if(ei0 != 0){
                                  m_vSSMGrid->at((ei0-1)+ej0*Lnx).edge_vrts[1] = vidx;
                                  m_vSSMGrid->at((ei0-1)+ej0*Lnx).num_ev++;
                                  if(bidx != -1){
                                          m_vSSMGrid->at((ei0-1)+ej0*Lnx).back_vrts[1] = bidx;
                                          m_vSSMGrid->at((ei0-1)+ej0*Lnx).num_bv++;
                                  }
                          }
                          if(ei0 != nx){
                                  m_vSSMGrid->at(ei0+ej0*Lnx).edge_vrts[3] = vidx;
                                  m_vSSMGrid->at(ei0+ej0*Lnx).num_ev++;
                                  if(bidx != -1){
                                          m_vSSMGrid->at(ei0+ej0*Lnx).back_vrts[3] = bidx;
                                          m_vSSMGrid->at(ei0+ej0*Lnx).num_bv++;
                                  }
                          }
                  }
  ...
  int CalMesh(int nx, int ny, std::vector<rxSSGrid> *grid, std::vector< std::vector<int> > &polys, int vstart,BboxScreenSpace Box)
    {
      	  int Lnx,Lny,Fnx,Fny;
    Fnx=Box.minx/screenSpace;
    Fny=Box.miny/screenSpace;
